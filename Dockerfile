...
RUN apt-get update && \
    ...
    libffi-dev \
    libopenblas-dev \
    # Rust/Cargo (for some Python package builds)
    cargo \
    # Download/archive tools & certs
    wget \
    tar \
    unzip \
    ca-certificates \
    # Autotools & libtool (for TA-Lib and other source builds)
    autoconf \
    automake \
    libtool \
    # CMake
    cmake \
    # LLVM 15 (for llvmlite 0.44.0)
    llvm-15 \
    llvm-15-dev \
    clang-15 \
    ...
    # Curl
    curl \
    # Git (useful for pnpm, some package installs)
    git && \
    # Clean up apt cache
...
RUN echo "Installing TA-Lib from source..." && \
    cd /tmp && \
    wget https://github.com/ta-lib/ta-lib/releases/download/v0.6.4/ta-lib-0.6.4-src.tar.gz -O ta-lib-0.6.4-src.tar.gz && \
...
RUN echo "Upgrading pip and installing prerequisites for vectorbt.pro..." && \
    python3 -m pip install --upgrade pip setuptools wheel --no-cache-dir --break-system-packages && \
    pip3 install pybind11 --no-cache-dir --break-system-packages && \
    pip3 install --ignore-installed llvmlite --no-cache-dir --break-system-packages && \
    pip3 install kaleido==0.1.0 --no-cache-dir --break-system-packages && \
    echo "Installing vectorbt.pro from source..." && \
    cd /usr/src/vectorbt.pro-main && \
    pip3 install ".[base]" --no-cache-dir --break-system-packages && \
    echo "vectorbt.pro installation complete."
